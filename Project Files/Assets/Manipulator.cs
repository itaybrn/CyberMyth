...
public class Manipulator : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        commandsToExecute = new List<PowerUpCommand>();

        GameObject[] objectsInLayer = GameObject.FindGameObjectsWithTag(tagName);
        players = GameObject.FindGameObjectsWithTag("Player");
        Debug.LogWarning("players size: " + players.Length);
    ...
    void SavePositions()
    {
        ...
        if (timeStop > 0)
        {
            foreach (TMObject TMObj in objects)
            {
            ...
            timeStop = Mathf.Max(0, timeStop - Time.deltaTime);
        }
        else
        {
            if (timer < ((float)1 / positionsPerSecond))
    ...
    {
        GameObject[] objectsInLayer = GameObject.FindGameObjectsWithTag(tagName);
        if (objectsInLayer.Length != objects.Count)
        {
            foreach (GameObject obj in objectsInLayer)
            {
                bool exists = false;
    ...
    public void savePowerUp(PowerUp powerUp, float parameter, int playerID)
    {
        ...
        if (PhotonNetwork.IsMasterClient)
            commandsToExecute.Add(newCommand);
    }
        ...
        if (command != null)
        {
            ...
            switch (command.type)
            {
                case PowerUp.TimeRewind:
                    ...
                    break;
                case PowerUp.TimeStop: //Ended up not using, though it is implemented
                ...
                case PowerUp.Superweapon:
                    ...
                    foreach (superweaponScript shot in SWShotArray)
                    {
                        if (player.transform.localScale.x < 0) //if the player faces left
                            shot.setDirection(true); //Superweapon Shot left
                        else
                            shot.setDirection(false); //Superweapon Shot right
                    }
                    break;
                case PowerUp.Clone:
    ...
    private void swapPositions(PowerUpCommand command, GameObject player)
    {
        ...
        if (index > objects.Count || index < 1)
            Debug.LogError("index: " + index + " out of range");
        else
        {
            TMObject obj = objects[index - 1];
            if (obj.destroyed)
